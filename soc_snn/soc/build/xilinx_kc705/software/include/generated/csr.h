...
#define CSR_EDABK_SNN_TICK_READY_ADDR (CSR_BASE + 0x44L)
#define CSR_EDABK_SNN_TICK_READY_SIZE 1
static inline uint32_t edabk_snn_tick_ready_read(void) {
	return csr_read_simple((CSR_BASE + 0x44L));
}
static inline void edabk_snn_tick_ready_write(uint32_t v) {
	csr_write_simple(v, (CSR_BASE + 0x44L));
}
#define CSR_EDABK_SNN_SNN_STATUS_ADDR (CSR_BASE + 0x48L)
...
static inline uint64_t sdmem2block_dma_base_read(void) {
	uint64_t r = csr_read_simple((CSR_BASE + 0x4000L));
	r <<= 32;
